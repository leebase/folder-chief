...
The email capability allows Folder Chief to search messages, read recent threads, summarize incoming mail, identify action items for `brain/state/today.md`, and compose reply text in this folder. It does not create remote drafts or send mail.
...
- **Strict in-folder draft rule**: Folder Chief is prohibited from creating remote drafts or sending email. It writes proposed reply text inside this folder; the human owner reviews, copies, and dispatches it through the mail client.
...
3. Ask Folder Chief to compose test reply text in a named in-folder draft file.
4. Verify that the file exists and that no remote email state changed.
5. Record the grant stanza and update the status to `active` with scope `read-only email; in-folder drafts` in `chief/capabilities.md`.
